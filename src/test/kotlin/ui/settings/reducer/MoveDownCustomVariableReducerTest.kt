...
class MoveDownCustomVariableReducerTest {

    val state = MutableStateFlow(SettingsState())
    val actionFlow = MutableSharedFlow<SettingsAction>()
    lateinit var reducer: MoveDownCustomVariableReducer
    ...
    fun setup() {
        ...
        reducer = MoveDownCustomVariableReducer(state, actionFlow)
    }

    @Test
    fun `when selected category not null on invoke`() = runBlockingTest {
        actionFlow.test {
            reducer.invoke(MoveDownCustomVariable(0))
